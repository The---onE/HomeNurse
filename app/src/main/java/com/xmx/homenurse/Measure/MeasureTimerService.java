...
public class MeasureTimerService extends Service {
    ...
    boolean getLatestPlan() {
        Cursor c = MeasureSQLManager.getInstance().getLatestSchedule();

        if (c.moveToFirst()) {
            latestId = MeasureSQLManager.getId(c);
            latestTime = MeasureSQLManager.getActualTime(c);
            latestTitle = MeasureSQLManager.getTitle(c);
            latestPlanTime = MeasureSQLManager.getPlanTime(c);
            latestRepeat = MeasureSQLManager.getRepeat(c);
            latestPeriod = MeasureSQLManager.getPeriod(c);
