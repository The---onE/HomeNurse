...
public class AddMeasureScheduleActivity extends BaseTempActivity {
    EditText titleText;
    EditText textText;
    ...
    protected void initView(Bundle savedInstanceState) {
        setContentView(R.layout.activity_add_measure_schedule);
        ...
        View time = lf.inflate(R.layout.content_add_schedule_by_time, null);
        View delay = lf.inflate(R.layout.content_add_schedule_by_delay, null);
    ...
    void insertPlan(String title, String text, Date date, int type, int repeat, int period) {
        long id = MeasureSQLManager.getInstance().insertSchedule(title, text, date, type, repeat, period);
        if (id >= 0) {
            showToast("添加成功");
            finish();
        } else {
